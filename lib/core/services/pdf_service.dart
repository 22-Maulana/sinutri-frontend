...
class PdfService {
  static Future<void> generateNutritionReport({
    ...
    required List<Map<String, dynamic>>
        childrenData, // List of {name: '', logs: [], growth: []}
  }) async {
          ...
          return [
            pw.Header(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text('Laporan Nutrisi & Pertumbuhan',
                      style: pw.TextStyle(
            ...
            // Mother Section
            pw.Text('PROFIL: $motherName',
                style: pw.TextStyle(
                    font: boldFont, fontSize: 16, color: PdfColors.teal)),
            pw.SizedBox(height: 10),
            _buildSummary(motherLogs, font, boldFont),
            pw.SizedBox(height: 10),
            pw.Text('Riwayat Konsumsi:',
                style: pw.TextStyle(font: boldFont, fontSize: 12)),
            _buildLogsTable(motherLogs, font, boldFont),
            pw.SizedBox(height: 15),
            pw.Text('Riwayat Pertumbuhan:',
                style: pw.TextStyle(font: boldFont, fontSize: 12)),
            ...
            for (var child in childrenData) ...[
              pw.Text('PROFIL: ${child['name']} (ANAK)',
                  style: pw.TextStyle(
                      font: boldFont, fontSize: 16, color: PdfColors.orange)),
              pw.SizedBox(height: 10),
    ...
    await Printing.layoutPdf(
      ...
      name:
          'Laporan_SINUTRI_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
    );
  }

  static pw.Widget _buildSummary(
      List<Map<String, dynamic>> logs, pw.Font font, pw.Font boldFont) {
    double totalCal = 0;
    double totalProt = 0;
    double totalCarbs = 0;
    ...
    return pw.Container(
      ...
      child: pw.Row(
        ...
        children: [
          _buildSummaryItem('Total Kalori',
              '${totalCal.toStringAsFixed(0)} kkal', boldFont, font),
          _buildSummaryItem('Total Protein', '${totalProt.toStringAsFixed(1)}g',
              boldFont, font),
          _buildSummaryItem('Total Karbo', '${totalCarbs.toStringAsFixed(1)}g',
              boldFont, font),
        ],
      ),
    );
  ...
  static pw.Widget _buildSummaryItem(
    ...
    return pw.Column(
      children: [
        ...
        pw.Text(value,
            style: pw.TextStyle(
                font: boldFont, fontSize: 14, color: PdfColors.teal700)),
      ],
    );
  }
  ...
  static pw.Widget _buildLogsTable(
      List<Map<String, dynamic>> logs, pw.Font font, pw.Font boldFont) {
    if (logs.isEmpty)
      return pw.Text('Belum ada data konsumsi.',
        ...
        for (var log in logs)
          pw.TableRow(
            children: [
              ...
              pw.Padding(
                  padding: const pw.EdgeInsets.all(5),
                  child: pw.Text(log['name'] ?? '-',
                      style: pw.TextStyle(font: font, fontSize: 8))),
              pw.Padding(
                  padding: const pw.EdgeInsets.all(5),
                  child: pw.Text('${log['calories']} kkal',
                      style: pw.TextStyle(font: font, fontSize: 8))),
              pw.Padding(
                  padding: const pw.EdgeInsets.all(5),
                  child: pw.Text('${log['protein']}g',
                      style: pw.TextStyle(font: font, fontSize: 8))),
            ],
          ),
      ],
  ...
  static pw.Widget _buildGrowthTable(
      List<Map<String, dynamic>> growth, pw.Font font, pw.Font boldFont) {
    if (growth.isEmpty)
      return pw.Text('Belum ada data pertumbuhan.',
          style: pw.TextStyle(font: font, fontSize: 10, color: PdfColors.grey));
        ...
        for (var record in growth)
          pw.TableRow(
            children: [
              ...
              pw.Padding(
                  padding: const pw.EdgeInsets.all(5),
                  child: pw.Text('${record['weight']}',
                      style: pw.TextStyle(font: font, fontSize: 8))),
              pw.Padding(
                  padding: const pw.EdgeInsets.all(5),
                  child: pw.Text('${record['height']}',
                      style: pw.TextStyle(font: font, fontSize: 8))),
              pw.Padding(
                  padding: const pw.EdgeInsets.all(5),
